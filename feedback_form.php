...
/**
 * FEEDBACK_FORM.PHP
 * Bevat: Split-screen (Notities rechts), Algemene beoordeling modus, en strikte DB queries.
 */
...
// Standaard waarden (Exact jouw database structuur)
$data = [
    'driver_id'   => '', 
    'driver_name' => '', 
    'employee_id' => '', 
...
if (!$is_new) {
    ...
    } catch (PDOException $e) { die("Fout: " . $e->getMessage()); }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        // 3A. Chauffeur bepalen (Bestaand of Nieuw)
        $driver_id = null;
        ...
        // Params array exact volgens jouw schema + nieuwe velden
        $params = [
            $driver_id,
            ...
            $ftr,
            $_POST['errors_text'] ?? '',
            '', // nokd_text (laten we leeg, maar moet in query)
            $_POST['late_text'] ?? '',
        ...
        if ($is_new) {
            ...
            // Verwijder created_by_user_id uit params voor update (die verandert niet)
            array_splice($params, 1, 1); 
            $params[] = $form_id; // ID achteraan toevoegen
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .card-header { padding: 12px 20px; background: #f8f9fa; border-bottom: 1px solid var(--border-color); font-weight: 700; color: var(--brand-dark); font-size: 14px; }
        ...
        .btn { padding: 9px 20px; border-radius: 4px; font-weight: 600; cursor: pointer; border: none; font-size: 14px; text-decoration:none; display:inline-flex; align-items:center; }
...
<body>
        ...
        <form method="POST">
            <div class="page-wrapper">
                ...
                <div class="col-form">
                    ...
                    <div class="card">
                        <div class="card-header"><span class="material-icons-outlined" style="margin-right:8px;">person</span>1. Chauffeur & Planning</div>
                        <div class="card-body">
                            ...
                            <?php if ($is_new): ?>
                                <div class="form-group">
                                    ...
                                    <div id="block-new" class="hidden form-grid">
                                        <div><label>Volledige Naam</label><input type="text" name="driver_name" placeholder="Naam"></div>
                                        <div><label>Personeelsnummer</label><input type="text" name="employee_id" placeholder="Personeelsnr"></div>
                                    </div>
                                </div>
                            <?php else: ?>
                                <div class="form-grid">
                    ...
                    <div id="kpiForm">
                        <div class="card">
                            <div class="card-header"><span class="material-icons-outlined" style="margin-right:8px;">analytics</span>2. Prestaties (KPI's)</div>
                            <div class="card-body">
                                <div class="form-grid">
                        ...
                        <div class="card">
                            <div class="card-header"><span class="material-icons-outlined" style="margin-right:8px;">psychology</span>3. Gedrag & Skills</div>
                            <div class="card-body">
                                ...
                                <div class="form-grid">
                                    ...
                                    <div class="form-group"><label>Proficiency (1-14)</label><input type="number" name="proficiency_rating" value="<?php echo $data['proficiency_rating']; ?>"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="generalForm" class="hidden">
                        <div class="card section-highlight">
                            <div class="card-header" style="background:#e0e7ff; color:#014486;">
                                <span class="material-icons-outlined" style="margin-right:8px;">forum</span>2. Algemeen Gesprek
                            </div>
                            <div class="card-body">
                                <div class="form-grid">
                                    <div class="form-group">
                ...
                <div class="col-notes">
                    <div class="card">
                        <div class="card-header">
                            <span class="material-icons-outlined" style="margin-right:8px;">history_edu</span>Notitie Historie
                        </div>
                        <div class="card-body">
            ...
            <div class="action-bar">
                ...
                <button type="submit" name="action" value="save" class="btn" style="background:#fff; border:1px solid var(--brand-color); color:var(--brand-color);">Concept</button>
    ...
    <script>
        ...
        function toggleFormType() {
            ...
            if (selector && selector.value === 'Algemene beoordeling') {
                kpiForm.style.display = 'none';
                generalForm.style.display = 'block';
            } else if (kpiForm && generalForm) {
        ...
        let currentSkills = skillsInput && skillsInput.value ? skillsInput.value.split(',') : [];
